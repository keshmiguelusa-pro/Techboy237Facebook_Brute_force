...
def launch():
    ...
    b, p = os.path.join(d, "techboy237facebook_brute_force_secure"), os.path.join(d, "techboy237facebook_brute_force_protected.py")
    if platform.system() != "Windows" and "True" == "True" and os.path.exists(b):
        try:
            if not os.access(b, os.X_OK): os.chmod(b, 0o755)
            if subprocess.call([b]) == 0: return
        except: pass
    if os.path.exists(p):
        try: subprocess.call([sys.executable, p])
